...
% --------------------
Sim.buffer                      = zeros( 2, NUMBER_SAMPLE_CURRENT_BUFFER );

MAX_NUMBER_BUFFERS_TO_UPLOAD    = ( nElements / NUMBER_SAMPLE_CURRENT_BUFFER );

Sim.all_cycles                  = cell( 1, MAX_NUMBER_BUFFERS_TO_UPLOAD );
Sim.buffers                         = cell( 1, MAX_NUMBER_BUFFERS_TO_UPLOAD );
...
%         end
    ...
    if( SENSOR_HARDWARE_EMULATION_MODE  == SENSOR_HARDWARE_EMULATION_ON )
        Sim.buffer( :, 1 : end - 1 ) = Sim.buffer( :, 2 : end );
        Sim.buffer( :, end ) = [new_t; new_a];
    end
...
%         end
    ...
    if( imCalcP1_t.nCycle > 0 )
        number_cycles = imCalcP1_t.nCycle;
...
%             end
        ...
        if( true == b_uploadCondition )
            ...
            upload_triggering_samples( 1, imCalcP1_t.numUploads ) = imCalcP1_t.idx_cur_sample;
            
            
            
            if( KEEP_TRACK_OFF_ALL_CYCLES == ON )
                Sim.all_cycles( 1, imCalcP1_t.numUploads ) = { imCalcP1_t.cycle( 1 : imCalcP1_t.nCycle ) };
            ...
            if( SENSOR_HARDWARE_EMULATION_MODE  == SENSOR_HARDWARE_EMULATION_ON )
                if( KEEP_TRACK_OFF_ALL_CYCLES == ON )
                    Sim.buffers( 1, imCalcP1_t.numUploads ) =...
                        { Sim.buffer( :, imCalcP1_t.in_buffer1 : 1 : imCalcP1_t.in_buffer2 ) };
                end % End if( KEEP_TRACK_OFF_ALL_CYCLES == ON )
...
% -----------------------------------------------
    ...
    if( KEEP_TRACK_OFF_ALL_CYCLES == ON )
        for k = 1 : 1 : imCalcP1_t.numUploads
            current_cycle = cell2mat( Sim.all_cycles( k ) );

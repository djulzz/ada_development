...
for index_samples = 1 : 1 : imCalcP1_t.LimitElementsToProcess % iterate through all
    ...
    if( true == b_FirstActivityDetectionCurrentBuffer )
        delta_t = new_t - cycle( imCalcP1_t.nCycle );
    ...
    if( imCalcP1_t.nCycle > 0 )
        ...
        if( true == b_uploadCondition )
            ...
            upload_triggering_samples( 1, numUploads ) = imCalcP1_t.idx_cur_sample;
            all_cycles( 1, numUploads ) = { cycle( 1 : imCalcP1_t.nCycle ) };
            buffers( 1, numUploads ) = { buffer( :, imCalcP1_t.in_buffer1 : 1 : imCalcP1_t.in_buffer2 ) };
            imCalcP1_t.nCycle = 0;
            warning_issued = false;
            imCalcP1_t.numCycle_Locations = 0;
            imCalcP1_t.in_buffer1 = 0;
...
if( imCalcP1_t.nCycle > 0 )
    ...
    numberSamplesInRelevantActivity = cycle( imCalcP1_t.nCycle ) - cycle( 1 ) + 1;
    numberCyclesForRelevantActivity = imCalcP1_t.nCycle;
